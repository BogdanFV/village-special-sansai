(() => {
  ...
  function initJs() {
    ...
    let seeksliderCovers = [
      ...postContainer.querySelectorAll('.post__seekslider__indicator'),
    ];
    ...
    function seektimeupdate(event) {
      ...
      seeksliderCovers[target__index].style.width =
        seeksliders[target__index].value + '%';
          ...
          switch (true) {
            case audio__currentTime >= 6 && audio__currentTime <= 1 + 16:
              ...
              if (Math.trunc(audio__currentTime) == 14) {
                subsContainer.children[currentIndexOfSub - 1].classList.remove(
                  'music-player-text',
                );
                subsContainer.children[currentIndexOfSub - 1].classList.add(
